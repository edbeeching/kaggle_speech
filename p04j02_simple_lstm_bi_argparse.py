...
def create_model(input_shape, num_neurons=128, regul=0.0):
    ...
    x = Bidirectional(LSTM(num_neurons, unroll=True))(model_input)
...
if __name__ == '__main__':    
    ...
    args = parser.parse_args(['128', '0.0'])
    ...
    tensor_board = TensorBoard(log_dir = 'tensorboard/model_p04j02_simple_lstm_argparse_N{}_R{}'.format(NUM_NEURONS, REG))
    check_point = ModelCheckpoint(filepath='models/model_p04j02_simple_lstm_argparse_N{}_R{}_'.format(NUM_NEURONS, REG) + '.{epoch:02d}.hdf5', period=10)
